...
const TableGroup = ({ tableInstance, className = '', customStyle, rowStyle }) => {
  ...
  return (
    <>
      <table className={className} {...getTableProps()} style={customStyle}>
        ...
        <tbody {...getTableBodyProps()} style={{ borderSpacing: '0 calc(0.75rem / 20* 7) !important' }}>
          {page.map((row, i) => {
            prepareRow(row);
            ...
            let hex = '';
            if (row.original?.producedProductSize) {
              const splitSubType = row.original?.producedProductSize?.split('Subtype')[0];
