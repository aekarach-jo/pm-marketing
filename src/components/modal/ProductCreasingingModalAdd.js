...
const ProductCreasingingModalAdd = ({ show, hide, setShowModal, condition = 'PRODUCT_CREASING', setValueChange, creasingLine }) => {
  ...
  const onFetchType = async (value) => {
    let ListType = {};
    ListType = await axios.get(`${SERVICE_URL}/masterData/lov/product/list?type=PRODUCT_CREASING`).then((res) => {
      if (show) {
        setIsFetch(true);
      } else {
        setIsFetch(false);
    ...
    const filteredData = ListType?.data?.filter((el) => {
      const hasNoMatch = !creasingLine?.some((itemValue) => {
        let code = false;
        if (itemValue.value === undefined) {
          code = el.code === itemValue;
        } else {
          code = el.code === itemValue.value;
        }
        return code;
      });
      return hasNoMatch;
  ...
  return (
    <Modal show={show} onHide={hide} size="lg">
      <Modal.Header closeButton>
        <Modal.Title>{f({ id: `product.creasing` })}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
...
export default ProductCreasingingModalAdd;
