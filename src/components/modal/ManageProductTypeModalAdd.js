...
const ManageProductTypeModalAdd = ({ show, hide, setShowModal, condition = 'productType' }) => {
  ...
  const onFetchType = async (value) => {
    let ListType = {};
    ListType = await axios.get(`${SERVICE_URL}/masterData/lov/product/list?type=PRODUCT_GROUP`).then((res) => {
      return res.data;
    });
  ...
  const handleOnSaveProductTypeItem = async (value) => {
    ...
    await axios.post(`${SERVICE_URL}/masterData/lov/product/save`, data, {
      headers: {
  ...
  return (
    <Modal show={show} onHide={hide} size="lg">
      <Modal.Header closeButton>
        <Modal.Title>{f({ id: `product.productGroup` })}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
                ...
                <>
                  {listData?.map((item, index) => (
                    <tr key={index} style={index % 2 === 0 ? { background: 'rgb(249, 249, 249)' } : {}}>
                      ...
                      <div className="w-10" style={{ width: '20px' }}>
...
export default ManageProductTypeModalAdd;
