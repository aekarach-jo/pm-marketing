...
const ManageSubTypeModalAdd = ({ show, hide, setShowModal, condition = 'PRODUCT_SUBTYPE', setValueChange, productSubType }) => {
  ...
  const onFetchType = async (value) => {
    let ListType = {};
    ListType = await axios.get(`${SERVICE_URL}/masterData/lov/product/list?type=PRODUCT_SUBTYPE`).then((res) => {
      return res.data;
    });
    ListType.data = ListType.data.filter((el) => productSubType !== el.code);
    if (show) {
      setIsFetch(true);
    } else {
      setIsFetch(false);
  ...
  return (
    <Modal show={show} onHide={hide} size="lg">
      <Modal.Header closeButton>
        <Modal.Title>{f({ id: `product.field.subType` })}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
...
export default ManageSubTypeModalAdd;
