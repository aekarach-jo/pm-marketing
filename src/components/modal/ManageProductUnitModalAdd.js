...
const ManageProductUnitModalAdd = ({ show, hide, setShowModal, condition = 'PRODUCT_BASEUOM', setValueChange, baseUOM }) => {
  ...
  const [obj, setObj] = useState({ abbr: '', code: '', name: '', type: 'PRODUCT_BASEUOM' });
  const [errCode, setErrorCode] = useState(false);
  const [isFetch, setIsFetch] = useState(false);

  const onFetchType = async (value) => {
    let ListType = {};
    if (value === 'PRODUCT_BASEUOM') {
      ListType = await axios.get(`${SERVICE_URL}/masterData/lov/product/list?type=PRODUCT_BASEUOM`).then((res) => {
        return res.data;
      });
    }
    ListType.data = ListType?.data.filter((el) => baseUOM !== el.code);
    setIsFetch(true);
  ...
  const onAdd = () => {
    ...
    if (checkCode) {
      ...
      setObj({ abbr: '', code: '', name: '', type: condition });
    }
  };
  ...
  return (
    <Modal show={show} onHide={hide} size="lg">
      <Modal.Header closeButton>
        <Modal.Title>{f({ id: `product.productUnit` })}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
...
export default ManageProductUnitModalAdd;
